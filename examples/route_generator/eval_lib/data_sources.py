...
class BenchmarkDataSource(DataSource):
    """A standard benchmark city (Mandl / Mumford). Init routes come from a
    saved dump when configured, else are generated by LC + the realistic tier.

    ``init_routes_path`` -- a plain route-tensor file (torch_utils.dump_routes
    format). ``init_dump`` -- a paper_results ``*_routes.pt`` dump; the entry
    whose label starts with "Initial" is used, so an experiment YAML can pin
    the exact machine-independent init network a previous run saved.
    """
    ...
    def _make_init(self, tensors, spec):
        ...
        if self.init_dump:
            ...
            dump = _torch.load(self.init_dump, map_location="cpu",
                               weights_only=False)
            routes = dump.get("routes", {})
            key = next((k for k in routes if str(k).startswith("Initial")), None)
            if key is None:
                raise ValueError(
                    f"init_dump {self.init_dump}: no 'Initial*' route set "
                    f"among {list(routes)}")
        ...
        if self.init_routes_path:
            from connectpt.routes_generator.torch_utils import load_routes_tensor
