...
def run_seeded_bee_colony(dataloader, eval_cfg, cost_obj, init_routes, *,
    ...
    block); ``plan`` is the ``ExecutablePlan`` that owns the bee taxonomy
    (counts, models, halt / max-len flags). ``init_routes`` is the starting
    network the search improves. The return value is exactly what ``test_method``
    returns (with ``return_routes=True`` and, when ``return_histories`` is set,
    the trailing cost-history list).
    """
