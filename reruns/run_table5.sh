#!/usr/bin/env bash
# Table 5 / Figure 5 (5 operator combos, Mumford1) rerun -> artifacts/reruns/. Detached (survives SSH disconnect).
...
setsid nohup python scripts/run_figure5_pareto.py --profile "$PROFILE" $SUITE_ARGS     > artifacts/cli_logs/rerun_table5.log 2>&1 < /dev/null &
PID=$!
sleep 3
echo "Table 5 / Figure 5 (5 operator combos, Mumford1) rerun started (PID $PID), profile=$PROFILE"
