"""Table 6-style random trim/extend ablation runner.

Runs two ten-bee configurations on the requested benchmark city/cities:
GNN + random trim/extend and Classic BCO + random trim/extend. Each per-city
CSV also contains Initial rows and the transfer metrics d0, d1, d2, d_un plus
total cost.

    python scripts/run_table6_random_trim_extend.py --profile full --suite suite_rerun
    python scripts/run_table6_random_trim_extend.py --profile smoke
    python scripts/run_table6_random_trim_extend.py --profile full --cities Mandl

Watch it online: tensorboard --logdir artifacts/runs/table6_random_trim_extend
"""
...
CONFIG = "table6_random_trim_extend"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=["smoke", "full"],
        default="smoke",
        help="smoke = 1-2 BCO iterations; full = the YAML/override budget",
    )
    parser.add_argument(
        "--suite",
        default="suite",
    ...
    log.info(
        "Random trim/extend ablation | profile=%s | config=%s | cities=%s | out=%s",
        args.profile,
        CONFIG,
        cities,
        out_dir,
    ...
    for city in cities:
        ...
        log.info("--- Random trim/extend ablation city=%s ---", city)
    ...
    log.info(
        "Random trim/extend ablation done. TensorBoard: tensorboard --logdir %s",
