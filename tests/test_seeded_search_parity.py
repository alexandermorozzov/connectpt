...
def _run_builder_path(spec, tensors, R):
    ...
    cfg = build_bco_cfg("parity_builder", spec["n_routes"], spec["min_route_len"],
                        spec["max_route_len"], use_neural_bees=False, n_bees=10,
...
def _run_library_native_path(spec, tensors, R):
    ...
    _mc, _sc, unserved, metrics, routes = run_seeded_bee_colony(
        ...
        device=torch.device("cpu"), silent=True)
    return _as_tensor(routes)


def test_library_native_seeded_bco_matches_builder_path():
    ...
    routes_builder = _run_builder_path(spec, tensors, R)
    routes_library = _run_library_native_path(spec, tensors, R)

    assert routes_builder.shape == routes_library.shape
    assert torch.equal(routes_builder, routes_library), (
        "library-native seeded BCO diverged from the builder path"
    )
